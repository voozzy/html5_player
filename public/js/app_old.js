...
// loadStream(decodeURIComponent(getURLParam('src',
  // 'http://109.87.171.129/dash/term2015.m3u8'
// )));
    ...
    hideCanvas();
     if(Hls.isSupported()) {
      ...
      $("#HlsStatus").text('loading ' + url);
       ...
       var cookiesAudLang = Cookies.get('hls_aud_lang');
       if (cookiesAudLang) {
         hls.defaultAudioLang = cookiesAudLang;
       } else {
		   try {
			   hls.defaultAudioLang = 'Russian';
		   } catch(e) {
			   
		   }
	   }

       var cookiesSubLang = Cookies.get('hls_sub_lang');
       if (cookiesSubLang) {
         hls.defaultSubtLang = cookiesSubLang;
       }
       ...
       var f = function(event,data) {
         hls.off(event, f);
         // $(audioLangs).val(hls.currentAudioLang);
         // $(subLangs).val(hls.currentSubtitleLang);
       };
       hls.on(Hls.Events.LEVEL_LOADED, f);
       hls.on(Hls.Events.MANIFEST_PARSED,function(event,data) {
       	 

       	 playerMain.initChangeAudio(hls.audioLangs);
       	 playerMain.initChangeQuality(generateQuality());
       	 
         // $.each(hls.audioLangs, function(number, audioLang) {
           // $(audioLangs).append($('<option>').text(audioLang).attr('value', audioLang));
         // });
         // $(audioLangs).change(function() {
           // hls.currentAudioLang = this.value;
           // if (this.value) {
             // Cookies.set('hls_aud_lang', this.value);
           // } else {
             // Cookies.remove('hls_aud_lang');
           // }
         // });

         // $.each(hls.subtitleLangs, function(number, subtitleLang) {
           // $(subLangs).append($('<option>').text(subtitleLang).attr('value', subtitleLang));
         // });
         // $(subLangs).change(function() {
           // hls.currentSubtitleLang = this.value;
           // if (this.value) {
             // Cookies.set('hls_sub_lang', this.value);
           // } else {
             // Cookies.remove('hls_sub_lang');
           // }
         // });
       });
