     ...
     */
     ...
     var ResolutionMenuButton = videojs.extend(MenuButton, {
       constructor: function(player, options, settings, label){
        this.sources = options.qualityList;
        this.label = label;
    ...
    videoJsResolutionSwitcher = function(options) {
      ...
      player.updateSrc = function(src){
        //Return current src if src is not given
	        ...
	        //Sort sources
	     	var choosen = options.defaultValue || options.qualityList[0];
	        var menuButton = new ResolutionMenuButton(player, { 
	        	sources: options.qualityList, 
      ...
      player.initQualityList = function(src){
      		if(player.controlBar.resolutionSwitcher){
	          player.controlBar.resolutionSwitcher.dispose();
	          delete player.controlBar.resolutionSwitcher;
	        }
			...
			videojs.addClass(menuButton.el(), 'vjs-resolution-button');
			$(menuButton.el()).click(function() {
				$('.playlistMenuWrap').not($('#settingsMenuWrap')).removeClass('active');
				$('#settingsMenuWrap').toggleClass('active');
			});
	        player.controlBar.resolutionSwitcher = player.controlBar.el_.insertBefore(menuButton.el_, player.controlBar.getChild('fullscreenToggle').el_);
	        player.controlBar.resolutionSwitcher.dispose = function(){
	          this.parentNode.removeChild(this);
