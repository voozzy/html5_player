...
function playList(options, arg, newCurrentIndex){
  var player = this;
  player.pl = player.pl || {};
  ...
  player.pl.init = function(videos, options, index) {
    options = options || {};
    player.pl.videos = [];
    player.pl.current = index || 0;
  ...
  player.pl._setVideoSource = function(src, poster) {
    // player.src(src);
    // loadStream(src[0].src);
    if (isInit) {
    	playerMain.playIndex(player.pl.current);
    	// alert(src[0].src);
    	// loadStream(src[0].src);
    	// hls.loadSource(src[0].src);
    }
    isInit = true;
  ...
  if (options instanceof Array){
    player.pl.init(options, arg, newCurrentIndex);
    player.pl._setVideo(newCurrentIndex || 0);
    return player;
  }
  ...
  else if (options === 'setIndex'){ 
    player.pl.current = arg;
  }
  else if (options === 'getCount'){ 
    return player.pl.videos.length;
  }
